...
# OUTPUT
# feasibility domain dataframe: "results/feasibility_domain_null.csv"
...
null.files <- list.files("results/null_fd/","*.csv")
if(length(null.files)>0){
  null.files.2 <- substr(null.files,9,nchar(null.files))
                                    ...
                                    .packages = c("tidyverse","foreach","matlib",
                                                  ...
                                                  "MultitrophicFun")) %dopar% {
                                                    ...
                                                    f <- function(m) class(try(solve(t(m) %*% m),
                                                                               silent = T))[[1]] == "matrix"
                                                    # null model to apply
                                                    # this is hard-coded to make it faster
                                                    # see get_network_metrics_v2 for the two null models
                                                    my.null <- "topology"
                                                    ...
                                                    for(i.null in 1:null.replicates){
                                                      ...
                                                      if(nrow(A)>2 & f(A) == "TRUE"){
                                                        omega.df <- try(feasibility_metrics(A = A,
                                                      ...
                                                      if(inherits(omega.df,"try-error")){
                                                        omega.df <- data.frame(omega_mean = NA,
                                                                               omega_lowerCI = NA,
                                                                               omega_upperCI = NA,
                                                                               omega_isotropic = NA,
                                                      ...
                                                      omega.df$null.replicate <- i.null
                                                      omega.df$null.type <- my.null
                                                      
                                                      null.reps.list[[i.null]] <- omega.df
                                                      
                                                    }
                                                    null.reps.df <- bind_rows(null.reps.list)
                                                    
                                                    # return
                                                    write.csv2(null.reps.df, file = paste("results/null_fd/fd_NULL_",id.char[i.id],"_",
