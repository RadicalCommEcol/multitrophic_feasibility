...
#' @param temp_dir directory where to store the temporary infomap files.
...
#'   
run_infomap_monolayer3 <- function(x, infomap_executable='Infomap', 
                                   temp_dir = "./",
                                   flow_model=NULL, 
                                   silent=F, 
                                   trials=100, 
  ...
  arguments <- paste(arguments,"-k")
  arguments <- paste(arguments,...)
  call <- paste("./",infomap_executable, paste(" ",temp_dir,'/infomap.txt . -i link-list --silent --tree --out-name ',temp_dir,"/infomap ",sep=""),arguments,sep='')
  
  # Write temporary file for Infomap
  write_delim(obs$edge_list_infomap, paste(temp_dir,'/infomap.txt',sep=""), delim = ' ', col_names = F)
  # Run Infomap
  if (verbose){ cat('running: ');cat(call);cat('\n') }
  system(call)
  tmp <- str_split(read_lines(paste(temp_dir,'/infomap.tree',sep=""))[5], 'levels')
  num_levels <- parse_number(tmp[[1]][1]) # Get number of levels
  
  L <- parse_number(read_lines(paste(temp_dir,'/infomap.tree',sep=""))[6]) # Get the map equation value, L
  # Read module results from Infomap
  modules <- suppressMessages(read_delim(paste(temp_dir,'/infomap.tree',sep=""), 
                                         delim = ' ', skip = 8, col_names = c('path', 'flow', 'name', 'node_id')))
  # Get the modules
  ...
  m <- max(modules$module_level1) # Get number of modules
  ...
  if (signif){
    ...
    L_sim <- m_sim <- NULL
    for (i in 1:nsim){
      write_delim(shuffled_linklist[[i]], paste(temp_dir,'/infomap.txt',sep=""), delim = ' ', col_names = F) # Write temporary file for Infomap
      if (verbose){ print(paste('Running Infomap on shuffled network ',i,'/',nsim,sep=''))}
      system(call) # Run Infomap
      L_sim <- c(L_sim, parse_number(read_lines(paste(temp_dir,'/infomap.tree',sep=""))[6])) # Get the map equation value, L
      m_sim <- c(m_sim, parse_number(str_split(read_lines(paste(temp_dir,'/infomap.tree',sep=""))[5], 'levels')[[1]][2])) # Get number of modules
    }
  ...
  file.remove(paste(temp_dir,'/infomap.txt',sep=""))
  file.remove(paste(temp_dir,'/infomap.tree',sep=""))
